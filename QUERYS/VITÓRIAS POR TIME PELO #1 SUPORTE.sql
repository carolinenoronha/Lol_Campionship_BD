...
WHERE bResult = 1 AND (blueSupport LIKE '%Loop%')) AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redSupport
FROM LeagueOfLegends 
WHERE rResult = 1 AND (redSupport LIKE '%Loop%')) AS Red) AS TABELA
...
(SELECT blueTeamTag AS Equipe, bResult, blueSupport as Suporte
FROM LeagueOfLegends 
WHERE blueTop LIKE '%Loop%') AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redSupport
FROM LeagueOfLegends 
WHERE redSupport LIKE '%Loop%') AS Red) AS TABELA
