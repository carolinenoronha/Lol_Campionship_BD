...
WHERE bResult = 1 AND (blueTop LIKE '%Mylon%')) AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redTop
FROM LeagueOfLegends 
WHERE rResult = 1 AND (redTop LIKE '%Mylon%')) AS Red) AS TABELA
...
(SELECT blueTeamTag AS Equipe, bResult, blueTop as Top
FROM LeagueOfLegends 
WHERE blueTop LIKE '%Mylon%') AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redTop
FROM LeagueOfLegends 
WHERE redTop LIKE '%Mylon%') AS Red) AS TABELA
