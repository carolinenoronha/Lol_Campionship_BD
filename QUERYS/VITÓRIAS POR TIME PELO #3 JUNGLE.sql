...
WHERE bResult = 1 AND (blueJungle LIKE '%Ranger%')) AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redJungle
FROM LeagueOfLegends 
WHERE rResult = 1 AND (redJungle LIKE '%Ranger%')) AS Red) AS TABELA
...
(SELECT blueTeamTag AS Equipe, bResult, blueJungle as Jungle
FROM LeagueOfLegends 
WHERE blueJungle LIKE '%Ranger%') AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redJungle
FROM LeagueOfLegends 
WHERE redJungle LIKE '%Ranger%') AS Red) AS TABELA
