...
WHERE bResult = 1 AND (blueMiddle LIKE '%tockers%')) AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redMiddle
FROM LeagueOfLegends 
WHERE rResult = 1 AND (redMiddle LIKE '%tockers%')) AS Red) AS TABELA
...
(SELECT blueTeamTag AS Equipe, bResult, blueMiddle as Mid
FROM LeagueOfLegends 
WHERE blueMiddle LIKE '%tockers%') AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redMiddle
FROM LeagueOfLegends 
WHERE redMiddle LIKE '%tockers%') AS Red) AS TABELA
