...
WHERE bResult = 1 AND (blueADC LIKE '%Matsukaze%')) AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redADC
FROM LeagueOfLegends 
WHERE rResult = 1 AND (redADC LIKE '%Matsukaze%')) AS Red) AS TABELA
...
FROM LeagueOfLegends 
WHERE blueADC LIKE '%Matsukaze%') AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redADC
FROM LeagueOfLegends 
WHERE redADC LIKE '%Matsukaze%') AS Red) AS TABELA
