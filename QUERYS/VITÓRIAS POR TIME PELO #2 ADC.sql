...
WHERE bResult = 1 AND (blueADC LIKE '%Skyer%')) AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redADC
FROM LeagueOfLegends 
WHERE rResult = 1 AND (redADC LIKE '%Skyer%')) AS Red) AS TABELA
...
FROM LeagueOfLegends 
WHERE blueADC LIKE '%Skyer%') AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redADC
FROM LeagueOfLegends 
WHERE redADC LIKE '%Skyer%') AS Red) AS TABELA
