...
WHERE bResult = 1 AND (blueMiddle LIKE '%Tinowns%')) AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redMiddle
FROM LeagueOfLegends 
WHERE rResult = 1 AND (redMiddle LIKE '%Tinowns%')) AS Red) AS TABELA
...
(SELECT blueTeamTag AS Equipe, bResult, blueMiddle as Mid
FROM LeagueOfLegends 
WHERE blueMiddle LIKE '%Tinowns%') AS Blue
UNION ALL 
SELECT * FROM
(SELECT redTeamTag AS Equipe, rResult, redMiddle
FROM LeagueOfLegends 
WHERE redMiddle LIKE '%Tinowns%') AS Red) AS TABELA
